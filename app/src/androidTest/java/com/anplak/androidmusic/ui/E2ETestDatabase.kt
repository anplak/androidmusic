...
object E2ETestDatabase {
    fun allTrackStats(context: Context): List<TrackStatsEntity> =
        runBlocking {
    ...
    fun cachedTrackCount(context: Context): Int =
        runBlocking {
            AppDatabase.getInstance(context).trackDao().getAll().size
        }
}

fun Map<Long, TrackStatsEntity>.playCountByTrack(): Map<Long, Int> = mapValues { it.value.playCount }

fun Map<Long, TrackStatsEntity>.skipCountByTrack(): Map<Long, Int> = mapValues { it.value.skipCount }

fun trackStatsSnapshot(context: Context): Map<Long, TrackStatsEntity> = E2ETestDatabase.allTrackStats(context).associateBy { it.trackId }
