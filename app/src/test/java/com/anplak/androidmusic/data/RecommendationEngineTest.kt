...
class RecommendationEngineTest {
    ...
    @Test
    fun `buildRows returns empty for empty library`() =
        runTest {
    ...
    @Test
    fun `buildRows includes daily mixes with stable slot ids`() =
        runTest {
    ...
    @Test
    fun `buildRows daily mixes avoid track overlap when possible`() =
        runTest {
            val library = richLibrary()
            val rows =
                engine.buildRows(
    ...
    @Test
    fun `buildRows includes at least one daily mix for sparse metadata library`() =
        runTest {
    ...
    @Test
    fun `buildRows includes because you listen row for top track`() =
        runTest {
    ...
    @Test
    fun `buildRows includes continue listening for last session`() =
        runTest {
    ...
    private class FakeStatsRepo : TrackStatsRepository {
        ...
        override suspend fun getStats(trackId: Long): TrackStats? = TrackStats(trackId, 0, null, 0, 0)
